...
public class CurrentStaysTablePanel extends JPanel {
    ...
    private CurrentStays currentStaysData;
    ...
    public CurrentStaysTablePanel(CurrentStays currentStaysData, User user, JLabel statusLabel) {
        this.currentStaysData = currentStaysData;
        ...
        loadStaysIntoTable(currentStaysData);

        add(new JScrollPane(staysTable), BorderLayout.CENTER);

        // --- Sorting Listener ---
        staysTable.getTableHeader().addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
        ...
        // --- Double-click to open Stay Details (optional) ---
        staysTable.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (e.getClickCount() == 2 && staysTable.getSelectedRow() != -1) {
                    int row = staysTable.getSelectedRow();
                    int stayId = (int) staysTable.getValueAt(row, 0);

                    Stay stay = currentStaysData.getStayById(stayId);

                    // TODO: Add StayDetailsScreen when ready
                    JOptionPane.showMessageDialog(null,
                            "Stay Details Coming Soon:\nStay ID: " + stay.getStayId());
                }
            }
        });
    }

    public void loadStaysIntoTable(CurrentStays currentStaysData) {
        tableModel.setRowCount(0);

        for (Stay stay : currentStaysData.getStayList()) {
    ...
    private void sortByColumn(int columnIndex) {
        ...
        currentStaysData.sortStaysBy(comparator);
        loadStaysIntoTable(currentStaysData);
    }
